...
export default function GuideClient() {
  ...
  if (!ready) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-neutral-950 text-sm text-neutral-400">
        Loading guide...
      </main>
    )
  }
        ...
        <section className="space-y-4 rounded border border-neutral-800 bg-neutral-900/60 p-5">
          ...
          <div className="space-y-4 text-sm leading-6 text-neutral-200">
            <div>
              ...
              <p className="text-neutral-300">Can the given passage answer the query clearly enough from the provided context?</p>
            </div>
            <div>
              <h3 className="font-medium text-white">Specificity (1–5)</h3>
              <p className="text-neutral-300">How specifically is the query tied to the provided passage and its context, rather than being vague or broadly applicable?</p>
            </div>
            <div>
              <h3 className="font-medium text-white">Query quality (1–5)</h3>
              <p className="text-neutral-300">How natural, useful, and realistic is the generated query as something a human might search?</p>
            </div>
            <div>
              <h3 className="font-medium text-white">Standalone clarity (1–5)</h3>
              <p className="text-neutral-300">Is the query understandable on its own, without missing context or hidden assumptions? References such as "this work", "these results", "the method", or "this approach" can reduce clarity if the missing referent is not clear from the query itself.</p>
            </div>
            <div>
              <h3 className="font-medium text-white">Scientific validity (1–5)</h3>
              <p className="text-neutral-300">Is the query scientifically faithful to the passage, without incorrect chemistry or misleading framing?</p>
            </div>
            <div>
              <h3 className="font-medium text-white">Top-10 retrieved relevance (1–3)</h3>
              <p className="text-neutral-300">For each non-gold retrieved passage in Task B, assign one relevance label: Not relevant, Somewhat relevant, or Relevant.</p>
            </div>
            <div>
              <h3 className="font-medium text-white">Near-miss</h3>
              <p className="text-neutral-300">A near-miss is a retrieved passage that is not the exact gold passage but is still strongly relevant and likely helpful for the same information need. If no retrieved passage has this property, leave the near-miss checkboxes empty. If one or more retrieved passages do have this property, check the corresponding retrieved items.</p>
            </div>
          </div>
        </section>
